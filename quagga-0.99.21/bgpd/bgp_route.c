            ...
            struct prefix_rd *prd, u_char *tag, int soft_reconfig)
{
  ...
  sdxext_bgp_update_bypass (peer, p, attr, afi, safi, type, sub_type, prd, tag,
          soft_reconfig);

  // REST OF CODE IS IN bgp_update_rsclients_bypass()
  /* should this intercept be up above, before bgp_update_main()? That wouldn't be
   * too hard to do (a bit of cut and paste, change of the returns to make everything
   * happy). Not totally cure.
   */
  /* END SDX EXTENSION CHANGE */
